...
use crate::vfs::{FileStat, FileSystem, FileType, Inode};
...
impl<D: BlockDevice + Send + Sync + 'static> NovaFS<D> {
    ...
    pub fn format(device: Arc<D>, block_offset: u32, total_blocks: u32) -> Arc<Self> {
        ...
        let data_area_blocks =
            total_blocks - 1 - inode_bitmap_blocks - data_bitmap_blocks - inode_area_blocks;

        // Generate simple random UUID and Key
        let mut uuid = [0u8; 16];
        let mut volume_key = [0u8; 32];
        let mut seed = crate::wall_clock();
        ...
        device
            ...
            .expect("format: failed to write superblock");
        ...
        for i in 0..(inode_bitmap_blocks + data_bitmap_blocks) {
            device
                .write_block(block_offset + 1 + i, zero_buf)
                .expect("format: failed to clear bitmap");
        }
        ...
        device
            .write_block(block_offset + 1, bitmap_buf)
            .expect("format: failed to write inode bitmap");

        // Reserve Metadata Blocks in Data Bitmap
        // We must mark all blocks before the Data Area as used, so alloc_block doesn't
        // return them.
        ...
        device
            .write_block(data_bitmap_start, data_bitmap_buf)
            .expect("format: failed to write data bitmap");

        // Write Root Inode
        let ts = crate::wall_clock();
        ...
        device
            .read_block(inode_start_block, buf)
            .expect("format: failed to read inode block");
        let inode_ptr = buf.as_mut_ptr() as *mut DiskInode;
        unsafe { *inode_ptr.add(1) = root_inode };
        device
            .write_block(inode_start_block, buf)
            .expect("format: failed to write root inode");
    ...
    fn alloc_inode(&self) -> Result<u32, &'static str> {
        ...
        for b in 0..self.sb.inode_bitmap_blocks {
            ...
            if let Err(e) = self.read_block(blk_id, buf) {
                println!(
                    "NovaFS Error: Failed to read inode bitmap block {}: {}",
                    blk_id, e
                );
            ...
            for i in 0..BLOCK_SIZE * 8 {
                ...
                if inode_idx >= max_inodes {
                    println!(
                        "NovaFS Error: Inode allocation reached limit {}",
                        max_inodes
                    );
                ...
                if (buf[i / 8] >> (i % 8)) & 1 == 0 {
                    ...
                    if let Err(e) = self.write_block(blk_id, buf) {
                        println!(
                            "NovaFS Error: Failed to write inode bitmap block {}: {}",
                            blk_id, e
                        );
    ...
    fn alloc_block(&self) -> Result<u32, &'static str> {
        ...
        for b in 0..self.sb.data_bitmap_blocks {
            ...
            if let Err(e) = self.read_block(blk_id, buf) {
                println!(
                    "NovaFS Error: Failed to read data bitmap block {}: {}",
                    blk_id, e
                );
                return Err("Bitmap read failed");
            }
            ...
            for i in 0..BLOCK_SIZE * 8 {
                if (buf[i / 8] >> (i % 8)) & 1 == 0 {
                    ...
                    if let Err(e) = self.write_block(blk_id, buf) {
                        println!(
                            "NovaFS Error: Failed to write data bitmap block {}: {}",
                            blk_id, e
                        );
                        return Err("Bitmap write failed");
                    }
                    ...
                    // `allocated` is the absolute device block number (the bit index in the
                    // data bitmap). Format marks bits 0..data_start-1 as metadata, so the
                    // first free bit is the first usable data block.
                    let abs_block = self.block_offset + allocated;

                    // Zero the block to prevent data leakage from previous usage
                    let zero_aligned = AlignedBlock([0u8; BLOCK_SIZE]);
                    if let Err(e) = self.write_block(abs_block, &zero_aligned.0) {
                        println!(
                            "NovaFS Error: Failed to zero allocated block {}: {}",
                            abs_block, e
                        );
    ...
    fn get_block_id(
        ...
        if (inner_id as usize) < DIRECT_POINTERS {
            if inode.direct[inner_id as usize] == 0 {
                ...
                // println!("NovaFS Debug: get_block_id allocated block {} for direct pointer
                // {}", new_block, inner_id);
                ...
                if check_aligned.0.iter().any(|&x| x != 0) {
                    println!(
                        "NovaFS Critical Error: Zeroing failed for block {}",
                        new_block
                    );
                    let mut dump = [0u8; 32];
            ...
            if (inner_id as usize) < indirect_limit {
                ...
                let ptrs = unsafe {
                    core::slice::from_raw_parts_mut(buf.as_mut_ptr() as *mut u32, ptrs_per_block)
                };
                ...
                let ptrs_l1 = unsafe {
                    core::slice::from_raw_parts_mut(buf_l1.as_mut_ptr() as *mut u32, ptrs_per_block)
                };
                ...
                let ptrs_l2 = unsafe {
                    core::slice::from_raw_parts_mut(buf_l2.as_mut_ptr() as *mut u32, ptrs_per_block)
                };
    ...
    fn get_inode_handle(
        self: &Arc<Self>,
        inode_id: u32,
    ) -> Result<Arc<NovaInode<D>>, &'static str> {
...
impl<D: BlockDevice + Send + Sync + 'static> Inode for NovaInode<D> {
    fn read_at(&self, offset: usize, buf: &mut [u8]) -> Result<usize, &'static str> {
        ...
        if encrypted {
            println!(
                "read_at: Encrypted flag set for inode {}",
                self.inode_number
            );
        }
        ...
        while curr < effective_end {
            ...
            buf[read_len..read_len + len]
                .copy_from_slice(&block_buf[offset_in_block..offset_in_block + len]);
    ...
    fn write_at(&self, offset: usize, buf: &[u8]) -> Result<usize, &'static str> {
        ...
        while written_len < buf.len() {
            ...
            block_buf[offset_in_block..offset_in_block + len]
                .copy_from_slice(&buf[written_len..written_len + len]);
    ...
    fn control(&self, op: u32, arg: u64) -> Result<u64, &'static str> {
        ...
        match op {
            ...
            2 => {
                ...
                log_debug!(
                    libnova::log::DOM_FS,
                    "NovaFS Debug: Control SetFlags inode {} = 0x{:x}",
                    self.inode_number,
                    inode.flags
                );
                self.fs.update_disk_inode(self.inode_number, &inode)?;
            ...
            7 => {
                // Set Flags (e.g. Encryption)
                log_debug!(
                    libnova::log::DOM_FS,
                    "NovaFS Debug: Control SetFlags (Op7) inode {} = 0x{:x}",
                    self.inode_number,
                    inode.flags
                );
                inode.flags = arg as u32;
    ...
    fn lookup(&self, name: &str) -> Result<Arc<dyn Inode>, &'static str> {
        ...
        // println!("NovaFS Debug: list inode {} size={} blocks={}", self.inode_number,
        ...
        while offset < size {
            ...
            if loop_count > MAX_LOOP {
                println!(
                    "NovaFS Error: lookup loop limit exceeded (offset={})",
                    offset
                );
            ...
            for i in 0..BLOCK_SIZE / entry_size {
                ...
                if entry.inode_number != 0 {
                    ...
                    if entry_name == name {
                        log_debug!(
                            libnova::log::DOM_FS,
                            "NovaFS Debug: lookup found '{}' (inode={}) in dir inode {}",
                            name,
                            entry.inode_number,
                            self.inode_number
                        );
    ...
    fn create(&self, name: &str, type_: FileType) -> Result<Arc<dyn Inode>, &'static str> {
        log_debug!(
            libnova::log::DOM_FS,
            "NovaFS Debug: create '{}' inside inode {}",
            name,
            self.inode_number
        );
        ...
        if new_inode_num == self.inode_number {
            println!(
                "NovaFS Critical: Allocator returned self inode {}! Bitmap corruption?",
                new_inode_num
            );
        ...
        if type_ == FileType::Directory {
            ...
            if let Err(e) = new_inode.add_dir_entry(".", new_inode_num) {
                // Cleanup
                println!(
                    "NovaFS Error: Failed to add '.' to new directory {}. Rolling back.",
                    new_inode_num
                );
                match self.remove(name) {
                    Ok(_) => {
                        if let Err(err) = self.fs.free_inode(new_inode_num) {
                            println!(
                                "NovaFS Error: Failed to free inode {} during rollback: {}",
                                new_inode_num, err
                            );
                        }
                    }
    ...
    fn list(&self) -> Result<Vec<(String, Arc<dyn Inode>)>, &'static str> {
        ...
        if size > MAX_LOOP * BLOCK_SIZE {
            println!(
                "NovaFS Warning: Directory size {} exceeds safety limit",
                size
            );
        }
        ...
        while offset < size {
            ...
            if entries.len() >= MAX_ENTRIES {
                println!(
                    "NovaFS Warning: Directory entry limit ({}) reached",
                    MAX_ENTRIES
                );
                break;
            }
            ...
            if offset < 2048 || size > 100000 {
                // println!("DEBUG: list dir offset={} block_idx={}
                // disk_block={}", offset, block_idx, disk_block);
            }

            let real_block = self.fs.block_offset + disk_block;
            // println!("NovaFS Debug: list reading block {} (real {})", disk_block,
            // real_block);

            // DEBUG: Check if block overlaps with file data (heuristic)
            // if offset > 4096 { println!("DEBUG: reading dir block at offset {} -> disk
            // {}", offset, disk_block); }
            ...
            for i in 0..BLOCK_SIZE / entry_size {
                ...
                let entry_ptr =
                    unsafe { block_buf.as_ptr().add(i * entry_size) as *const DirEntry };
                ...
                if entry.inode_number != 0 {
                    ...
                    match self.fs.get_inode_handle(entry.inode_number) {
                        ...
                        Err(_e) => {
                            // Suppress log to avoid spam during
                            // recovery/listing
                        }
                    }
                }
    ...
    fn link(&self, name: &str, other: &dyn Inode) -> Result<(), &'static str> {
        ...
        // Lock metadata to check type and update nlink
        {
            ...
            target_data.nlink += 1;
            target_data.ctime = crate::wall_clock();
    ...
    fn rename(
        ...
        if let Ok(dest_inode) = new_parent_handle.lookup(new_name) {
            ...
            if target_stat.file_type == FileType::Directory {
                if dest_stat.file_type != FileType::Directory {
                    return Err("Not a directory"); // Target is a file, source
                                                   // is dir
                }
                ...
                if dest_stat.file_type == FileType::Directory {
                    return Err("Is a directory"); // Target is dir, source is
                                                  // file
                }
            }
        ...
        // We use internal helper to avoid "Directory not empty" check and nlink
        // decrement
    ...
    fn remove(&self, name: &str) -> Result<(), &'static str> {
        log_debug!(
            libnova::log::DOM_FS,
            "NovaFS Debug: remove request for '{}' (len={}) in inode {}",
            name,
            name.len(),
            self.inode_number
        );
        ...
        while offset < size {
            ...
            for i in 0..BLOCK_SIZE / entry_size {
                ...
                let entry_ptr =
                    unsafe { block_buf.as_mut_ptr().add(i * entry_size) as *mut DirEntry };
                ...
                if entry.inode_number != 0 {
                    ...
                    // println!("NovaFS Debug: remove scanning '{}' (match target '{}'?)",
                    // entry_name, name);
...
impl<D: BlockDevice + Send + Sync + 'static> NovaInode<D> {
    fn add_dir_entry(&self, name: &str, inode_num: u32) -> Result<(), &'static str> {
        ...
        log_debug!(
            libnova::log::DOM_FS,
            "NovaFS Debug: adding entry '{}' (inode={}) to dir inode {}",
            name,
            inode_num,
            self.inode_number
        );
        ...
        if let Err(e) = self.fs.read_block(real_block, block_buf) {
            println!(
                "NovaFS Error: add_dir_entry read target block failed: {}",
                e
            );
            return Err("Read failed");
        }
        ...
        if !found_free {
            ...
            log_debug!(
                libnova::log::DOM_FS,
                "NovaFS Debug: add_dir_entry updating inode {} size to {}",
                self.inode_number,
                parent_inode.size
            );
            if let Err(e) = self.fs.update_disk_inode(self.inode_number, &parent_inode) {
                ...
                for i in 0..ptrs_per_block {
                    ...
                    if l2_block_idx != 0 {
                        ...
                        for j in 0..ptrs_per_block {
                            let data_block =
                                unsafe { *(l2_buf.as_ptr().add(j as usize * 4) as *const u32) };
                            if data_block != 0 {
            ...
            for i in 0..BLOCK_SIZE / entry_size {
                ...
                if entry.inode_number != 0 {
                    ...
                    if entry_name == name {
                        ...
                        if offset + entry_size >= size {
                            parent_inode.size = offset as u32;
                            // TODO: ideally we should scan backwards to find
                            // the new last entry to shrink further,
                            // but shrinking by one entry is a good start to
                        ...
                        {
                            println!(
                                "NovaFS Error: remove_entry_internal failed to update inode: {}",
                                e
                            );
                        }
