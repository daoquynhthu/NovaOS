...
impl<'a> ElfLoader<'a> {
    ...
    pub fn load_elf<A: ObjectAllocator>(
        ...
        let mut aligned_buf = AlignedElfBuf([0; MAX_ELF_SIZE]);
        let elf_data_aligned: &[u8] = if (elf_data.as_ptr() as usize).is_multiple_of(core::mem::align_of::<u64>()) {
            elf_data
        } else {
            aligned_buf.0[..elf_data.len()].copy_from_slice(elf_data);
            &aligned_buf.0[..elf_data.len()]
        };
